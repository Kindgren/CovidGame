...
//	private PlayState playState;
	...
	//	playState = new PlayState(model,0,0,100);
		...
		MenuButton newGame = new MenuButton(710,50, 150, 50, "New Game");
		MenuButton highScores = new MenuButton(710,150, 150, 50, "Highscores");
		MenuButton exit = new MenuButton(710,250, 150, 50, "Exit Game");
		buttons[0]=newGame;
	...
	//	drawBg(g, bgColor);
		...
		g.fillText(informationText, 190, 90);
		g.strokeText(informationText, 190, 90);
	...
	public void mousePressed(MouseEvent event) {
		...
		for (MenuButton b : buttons) {
			if (mx>=b.getX()&& mx <= b.getX()+b.getWidth()) {
				...
				//model.switchState(playState);
				model.switchState(new PreGameState(model,bg));	
				}
				if(b.getLabel()=="Highscores") {
				model.switchState(new HighScoreState(model,null));	
					}
